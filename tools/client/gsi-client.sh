...
command_divider()
{
        echo $(date +"%FT%T"): " " >>  ~/$clientgsidir/gsi_client.log
        echo $(date +"%FT%T"): "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" >>  ~/$clientgsidir/gsi_client.log
	echo $(date +"%FT%T"): "command: ${*}" >>  ~/$clientgsidir/gsi_client.log
	echo $(date +"%FT%T"): " " >>  ~/$clientgsidir/gsi_client.log
}
...
netstat -Wan |tee netstat_Wan >> gsi_client.log
...
then
	...
	command_divider "No sudo permission.  Several commands will not be caputed."
fi
command_divider "mount |egrep lustre; mount"
mount |egrep lustre |tee mount >> gsi_client.log; mount |tee -a mount >> gsi_client.log
if [ -f /etc/fstab ]
then
